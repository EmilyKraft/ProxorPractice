...
public class SpreadSheet extends JFrame {
    ...
    public void evaluate(int r, int c, int depth) {
        ...
        if ((formula.length() > 8) && (formula.charAt(0)!='=')){
        	cells[r][c].fullValue = formula;
        	formulaBar.setText(cells[r][c].fullValue);
        	cellsTF[r][c].setText(df.format(Double.parseDouble(formula)));
            ...
            try {
                if (depth <= maxRows * maxCols) {
                    ...
                    if (tokens.hasMoreTokens() && 
                        ...
                        if (val != null) {
                        	if (val.length() > 8){
                        		cells[r][c].fullValue=val;
                        		formulaBar.setText(cells[r][c].fullValue);
                            	cellsTF[r][c].setText(df.format(Double.parseDouble(val)));
                            	cells[r][c].valid = true;
                            	return;
                        	}
                        	else{
    ...
    public void cellChanged(JTextField cell) {
        ...
        if (f.length() <= 0 || f.charAt(0) != '=') {
            ...
            if (!ignoreTextFieldAction) {
                formulaBar.setText(cells[r][c].formula);
            }
        }
        evaluate();
    }
    ...
    private class CellFocusListener implements FocusListener {
        public void focusGained(FocusEvent e) {
            ...
            editRow = loc.x;
            editCol = loc.y;
            formulaBar.setText(cells[editRow][editCol].formula);
        }
    ...
    private class FormulaActionListener implements ActionListener {
        public void actionPerformed(ActionEvent e) {
            cells[editRow][editCol].formula = formulaBar.getText();
            evaluate();
        }
    }
    ...
    public void addComponentsToPane(final Container pane) {
        ...
        formulaBar.addActionListener(formulaActionListener);
        pane.add(formulaBar, BorderLayout.NORTH);
