...
public class SpreadSheet extends JFrame {
	...
	protected String sum(String x, String y, int depth){
		double total = 0;
		char xC = x.charAt(0);
		char yC = y.charAt(0);
		char xR = x.charAt(1);
		...
		if (xC>yC){
			for (int i = yC; i <= xC; i++){
				String token = "" + (char)i + (char)xR;
				String data = evaluateToken(token, depth);
				if (data.equals(null)){
					return "!!!";
				}
				else{
